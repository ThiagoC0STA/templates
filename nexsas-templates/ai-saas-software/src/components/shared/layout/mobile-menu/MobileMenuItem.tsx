...
const MobileMenuItem = ({ id, title, children, hasSubmenu = false }: MobileMenuItemProps) => {
  ...
  return (
    <li className="space-y-2">
      ...
      >
        <span className="text-background-6 text-tagline-1 block font-normal">{title}</span>
        {hasSubmenu && (
          <span
      ...
      {hasSubmenu && children && (
        <ul
          ...
          data-submenu={id}
          className={cn(
            'mobile-submenu block w-full overflow-y-hidden transition-[height,opacity] duration-300',
