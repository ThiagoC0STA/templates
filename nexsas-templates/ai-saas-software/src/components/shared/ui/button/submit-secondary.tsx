...
export interface SubmitSecondaryProps extends Omit<
...
const SubmitSecondary: FC<SubmitSecondaryProps> = ({
  children,
  className,
  type = 'submit',
  ...rest
  ...
  const baseClass =
    'group/btn-secondary cursor-pointer bg-background-7 hover:border-stroke-3/20 hover:bg-background-7/95 relative flex h-full max-h-12 items-center justify-center gap-2 overflow-hidden rounded-[4px] border border-transparent px-6 py-[13px] transition-all duration-300 ease-in-out';
